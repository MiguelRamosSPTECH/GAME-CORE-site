function cadastrar() {
    ...
    msg_erro.innerHTML = "Preencha todos os campos!"
      ...
      else if (nomeEmpresarialVar.length < 3) {
        alert(`Insira um nome empresarial válido! Nome muito pequeno.`)
    }
     else if (cnpjVar.length != 14){
        alert(msg_erro.innerHTML = `Insira um CNPJ válido!`)
    }
     else if (nomeRepresentanteVar.length < 3) {
        alert(`Insira um nome de representante válido! Nome muito pequeno.`)
    }
     else if (emailVar.length < 5) {
        alert(`Preencha um email válido! Email muito pequeno.`)
    }
     else if (!emailVar.includes("@") || !emailVar.includes(".")) {
        alert(`Insira um e-mail válido! Precisa ter "@" e "."`)
    }
     else if (cpfVar.length != 11){
        alert(`Insira um CPF válido!`)
    }
     else if (senhaVar.length < 5) {
        alert(`Insira uma senha válida! Senha muito curta.`)
    }
    else {
